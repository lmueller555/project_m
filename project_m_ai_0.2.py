...
# Load the dataset
data = pd.read_csv(
    'C:\\Users\\lmueller\\Desktop\\Project M\\Project_M_Statistical_Data_Predictions.csv')
...
MAX_TRADE_QUANTITY = 20  # This can be adjusted by the AI as part of its learning process
...
# After identifying pairs and calculating normal ratios...
# print("Identified pairs and their normal ratios:")
for pair in pairs:
    print(pair)
...
# Displaying the unique company names after preprocessing
# print("Unique Company Names in Dataset:", data['Company'].unique())

# Function to reformat pair names to match the preprocessed company names
...
for generation in range(num_generations):
    ...
    for trading_day in unique_trading_days:
        ...
        # Apply pair trading using the aggregated data
        pair_trading(aggregated_day_data, portfolio, formatted_pairs,
                     normal_ratios, entry_threshold, exit_threshold)
...
plt.xlabel('Stocks')
plt.ylabel('Quantity')
plt.title('Portfolio Composition of the Last Generation')
plt.xticks(rotation=45)
plt.tight_layout()
plt.show()

# After all generations, plot the ROIs
plt.figure(figsize=(14, 7))
plt.plot(range(num_generations), generation_rois, marker='o')
plt.title('ROI per Generation')
