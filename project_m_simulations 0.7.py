...
class TradingAI:
    ...
    def calculate_rolling_correlations(self, window_size=30):
        pivot_df = self.dataset.pivot(
            index='Date', columns='Company', values='Close/Last')
        rolling_corr = pivot_df.rolling(
            window=window_size).corr().dropna(how='all', axis=1)
        # Aggregate correlations by taking the last available correlation matrix for the window
        corr_by_date = {date: rolling_corr.xs(
            date, level='Date') for date in rolling_corr.index.get_level_values('Date').unique()}
        return corr_by_date
    ...
    def trade_using_correlations(self, current_date):
        ...
        if current_date in self.correlations:
            ...
            for company, shares in self.shares_held.items():
                if shares > 0:
                    correlated_companies = corr_matrix[company].drop(
                        company).sort_values(ascending=False)

                    # Example logic to sell stocks that are highly correlated with stocks that are dropping
                    for corr_company, corr_value in correlated_companies.items():  # Replaced iteritems() with items()
                        if corr_value > 0.8:  # Arbitrary threshold for high correlation
                            if self.dataset[(self.dataset['Date'] == current_date) & (self.dataset['Company'] == corr_company)]['Close/Last'].iloc[0] < self.purchase_prices[corr_company]:
                                self.sell_stock(corr_company, self.dataset[(self.dataset['Date'] == current_date) & (
                                    self.dataset['Company'] == corr_company)]['Close/Last'].iloc[0], "Correlated Drop")
    ...
    def trade(self, day, parameters):
        ...
        for index, row in day_data.iterrows():
            ...
            # Checking the crossover condition for buying
            if crossover and deviation < parameters['upper_deviation'] and self.cash >= current_price:
            ...
            # Checking the crossover condition for selling
            if not crossover and self.shares_held[company] > 0:
                self.sell_stock(company, current_price, "MA Crossover")
...
correlation_dataset_path = 'C:\\Users\\lmueller\\Desktop\\Project M\\Compiled_Companies_Data_with_30_MA.csv'
